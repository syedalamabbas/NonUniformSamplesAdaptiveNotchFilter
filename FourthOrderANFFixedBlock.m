function [X1,X2,M]  = FourthOrderANFFixedBlock (Y, T, initialFreq, gamma, xi)
...
%      Y(i) = Y(i)/ ( sqrt(X1(i)^2 + X2(i)^2/ M(i)^2) * 2*xi );  % Normalizing Amplitude  
...
% for m = 1:55       % Multiple Iterations
% %      Y = Y / mean (abs(Y));
% %     Y = 2* Y - X2 ./ M * 2*xi; 
%       Y =  Y / mean(sqrt(X1(1:N).^2 + (X2(1:N)./ M(1:N)).^2) * 2*xi );  % Normalizing Amplitude 
% 
% for i =2:N
%     %      DY = [Y(i)-Y(i-1)+T^2*M(i-1)^2*Y(i-1)/2 + T^4*M(i-1)^4*Y(i-1)/24]/(T - M(i-1)^2*T^3/6);      %  Using Backward Differentiation Formula
% %     DY = [Y(i)-Y(i-1)*(1 - T^2*M(i-1)^2/2 + T^4*M(i-1)^4/24) ]/(T-M(i-1)^2*T^3/6);
