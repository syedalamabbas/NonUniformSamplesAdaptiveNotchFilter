...
Y_Estimated_Derivatives = zeros (1,N);
...
%     DY = [Y(i)-Y(i-1)*(1 - T^2*M(i-1)^2/2 + T^4*M(i-1)^4/24) ]/(T-M(i-1)^2*T^3/6);
    Y_Estimated_Derivatives(i) = [Y(i)-Y(i-1)*(1 - T^2*M(i-1)^2/2 + T^4*M(i-1)^4/24) ]/(T-M(i-1)^2*T^3/6);
%     if (i > 2)
%         del_T = double(modifiedt(i-1)- modifiedt(i-2));
%         DY_2 = [Y(i-2)-Y(i-1)*(1 - del_T^2*M(i-1)^2/2 + del_T^4*M(i-1)^4/24) ]/(-del_T+M(i-1)^2*del_T^3/6);
%         DY = (DY+DY_2)/2;
%     end
    ...
    M(i) = M(i-1) + T*DM + T^2/2*D2M + T^3/6*D3M+ T^4/24*D4M;

    %------------------------------------------------------------------%
end

%--------Plotting the derivative of the signal in ahighly noisy enviornment
% A =  1;
% phi = pi/2;
%  f =  60;
%  
...
% 
% figure, plot (modifiedt, Y_derivative, modifiedt, Y_Estimated_Derivatives,'LineWidth' , 1.5)
% Ax = legend ('True 1^{st} Derivative of y', 'Numerical Differentiator, Dy')
% Ax.FontSize = 14;
